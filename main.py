...
for epoch in tqdm(range(args.learning_epoch_num)):
    ...
    if args.use_discriminator:
        ...
        # エポックの終わりにログを記録
        avg_generator_loss = np.mean(losses_train_g)
        avg_discriminator_loss = np.mean(losses_train_d)
        ...
        logging.info(f"Epoch {epoch+1}/{args.learning_epoch_num} : Generator loss = {avg_generator_loss}, Discriminator loss = {avg_discriminator_loss}")
    ...
    # Discriminatorを使わない場合
        ...
        fig_pred, fig_target = plot_target(pred), plot_target(target)
        writer.add_figure('Prediction', fig_pred, epoch)
        writer.add_figure('Target', fig_target, epoch)
        ...
        if epoch % 10 == 0:
            ...
            fig_pred.savefig(f"{fig_dir}/prediction_{epoch}.png"), fig_target.savefig(f"{fig_dir}/target_{epoch}.png")
