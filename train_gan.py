...
generator.load_state_dict(torch.load("result/2024-01-26-03-13/model/generator_40.pth", map_location=device))

#StockPriceEstimatorのgeneratorのパラメタのみ更新
...
# generatorのパラメータを表示、パラメタ更新の対象になっているか確認
for name, param in generator.named_parameters():
    if param.requires_grad:
        print(name)
...
for epoch in tqdm(range(args.learning_epoch_num)):
    ...
    for j, data in enumerate(train_dataloader):
        ...
        # Generatorに入力
        #print(price.shape, text.shape)
        pred = generator(price, text)
        # plt.plot(pred[0, :, 0].cpu().detach().numpy())
        # plt.fill_between(range(len(pred[0, :, 0].cpu().detach().numpy())), pred[0, :, 1].cpu().detach().numpy(), pred[0, :, 2].cpu().detach().numpy(), color="blue", alpha=0.3)
        # plt.show()
        # Discriminatorを欺く方向に学習
        d_fake = discriminator(torch.permute(pred, (0,2,1)))
        # 損失の計算
        loss_train_g = criterion(d_fake, torch.ones_like(d_fake))#loss_train_bce_g
        # 誤差逆伝播
    ...
    plt.legend()
    plt.title(f"Prediction and Target, RMSE: {avg_rmse_valid_g}")

    writer.add_figure('Fig_valid', fig, epoch)

    logging.info(f"Epoch {epoch+1}/{args.learning_epoch_num} : Training loss = {avg_loss_train_g}, Validation loss = {avg_rmse_valid_g}")
